  ...
  run: async (client, m, args, usedPrefix, command, text) => {
    try {
      ...
      if (url) {
        ...
        if (!detail.detalles)
          return client.reply(m.chat, `《✧》 No se pudo obtener el pack desde la URL.`, m);
        
        packData = detail.detalles;
      } else {
        const search = await searchPacks(text);
        if (!search.status || !search.resultados?.length)
          return client.reply(m.chat, `《✧》 No se encontraron packs para *${text}*.`, m);
      ...
      const MAX_STICKERS = 50; // Puedes bajarlo a 30 si prefieres
      ...
      if (!stickerResults.length)
        return client.reply(m.chat, `《✧》 No se pudieron procesar los stickers del pack.`, m);
      
      await client.sendMessage(m.chat, {
        stickerPack: {
          name: packName,
          publisher: author?.name || author?.username || `@${name}`,
          description: 'Ig: Lzzin.xyz',
          cover,
      ...
      console.error(e);
      await m.react('✖️');
      return m.reply(`> Ocurrió un error inesperado al ejecutar *\( {usedPrefix + command}*.\n> [Error: * \){e.message}*]`);
    }
  }
};
...
async function toVideoNote(url) {
  ...
  const tmpIn = `/tmp/ptv_in_${timestamp}.mp4`;
  const tmpOut = `/tmp/ptv_out_${timestamp}.mp4`;
  ...
  try {
    ...
    // Convertir a video note (círculo, máximo 60 segundos)
    await execAsync(
      `ffmpeg -y -threads 1 -i ${tmpIn} -t 60 ` +
      `-vf "crop=min(iw\\,ih):min(iw\\,ih),scale=480:480" ` +
      `-c:v libx264 -preset ultrafast -crf 28 ` +
      `-c:a aac -b:a 64k -movflags +faststart ${tmpOut}`
    );
...
// Precarga opcional (sin caché persistente)
